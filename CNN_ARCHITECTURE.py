...
# In case of testing you can use sample of the data to train the model faster :
...
df.drop(columns=["Unnamed: 0", "name_reviewer", "country_reviewer", "airline_name", "section_being_reviewed", "date_review", "review_title", "verified_trip",
                 "review_count_reviewer", "date_response", "text_response", "Type Of Traveller", "Seat Type", "Route", "Date Flown"], inplace=True)
...
conv1 = Conv1D(128, 3, activation='relu')(x)
conv2 = Conv1D(128, 4, activation='relu')(x)
conv3 = Conv1D(128, 5, activation='relu')(x)
# Apply max pooling to each of the convolutional layers
maxpool1 = GlobalMaxPooling1D()(conv1)
maxpool2 = GlobalMaxPooling1D()(conv2)
maxpool3 = GlobalMaxPooling1D()(conv3)
# Concatenate the output from each max pooling layer
merged = concatenate([maxpool1, maxpool2, maxpool3])
