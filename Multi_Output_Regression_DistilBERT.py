...
from keras.layers import Input, Embedding, Conv1D, MaxPooling1D, GlobalMaxPooling1D, Dense, concatenate, SpatialDropout1D, Dropout, LSTM, Embedding, Concatenate, Flatten, Dense
...
X_train, X_test, y_train, y_test = train_test_split(df['decontracted'], df[['Seat Comfort', 'Cabin Staff Service', 'Food & Beverages', 'Ground Service', 'Inflight Entertainment',
                                                                            'Value For Money', 'Wifi & Connectivity', 'user_rating']], test_size=0.2, random_state=42)
...
output1 = tf.keras.layers.Dense(1, activation='linear', name='Seat_Comfort')(pooler_output)
output2 = tf.keras.layers.Dense(1, activation='linear', name='Cabin_Staff_Service')(pooler_output)
output3 = tf.keras.layers.Dense(1, activation='linear', name='Food_Beverages')(pooler_output)
output4 = tf.keras.layers.Dense(1, activation='linear', name='Ground_Service')(pooler_output)
output5 = tf.keras.layers.Dense(1, activation='linear', name='Inflight_Entertainment')(pooler_output)
output6 = tf.keras.layers.Dense(1, activation='linear', name='Value_For_Money')(pooler_output)
output7 = tf.keras.layers.Dense(1, activation='linear', name='Wifi_Connectivity')(pooler_output)
output8 = tf.keras.layers.Dense(1, activation='linear', name='user_rating')(pooler_output)

model = tf.keras.models.Model(inputs=[input_ids, attention_mask], outputs=[output1, output2, output3, output4, output5, output6, output7, output8])
...
with tf.device('/device:GPU:0'):
    model.fit(
        [train_input_ids, train_attention_mask], [y_train['Seat Comfort'], y_train['Cabin Staff Service'], y_train['Food & Beverages'], y_train['Ground Service'],
                                                  y_train['Inflight Entertainment'], y_train['Value For Money'], y_train['Wifi & Connectivity'], y_train['user_rating']],
        validation_data=([test_input_ids, test_attention_mask], [y_test['Seat Comfort'], y_test['Cabin Staff Service'], y_test['Food & Beverages'], y_test['Ground Service'],
                                                                 y_test['Inflight Entertainment'], y_test['Value For Money'], y_test['Wifi & Connectivity'], y_test['user_rating']]),
        epochs=10, batch_size=16
    )

# Evaluate the model
with tf.device('/device:GPU:0'):
    scores = model.evaluate([test_input_ids, test_attention_mask], [y_test['Seat Comfort'],
                            y_test['Cabin Staff Service'], y_test['Food & Beverages'], y_test['Ground Service'], y_test['Inflight Entertainment'],
                            y_test['Value For Money'], y_test['Wifi & Connectivity'], y_test['user_rating']])

labels_array = ['Seat Comfort', 'Cabin Staff Service', 'Food & Beverages', 'Ground Service', 'Inflight Entertainment', 'Value For Money',
                'Wifi & Connectivity', 'user_rating']
index = 0
...
#----------------------------------------------------------------------------------------- Feature Importance --------------------------------------------------------------------------------------
# Calculate the importance as the sum of absolute values of weights for each feature :# Load the feature names from the file
...
#print(embedding_weights_sum[:52])
